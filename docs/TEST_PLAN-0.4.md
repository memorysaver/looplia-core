...
> **Related:** [CHANGELOG.md](../CHANGELOG.md) | [AGENTIC_CONCEPT-0.2.md](./AGENTIC_CONCEPT-0.2.md) | [GLOSSARY.md](./GLOSSARY.md)

This document describes the test architecture for Looplia-Core v0.5.1, aligned with Clean Architecture principles and the Workflow-as-Markdown design.
...
| **Integration** | Core services | Workflow orchestration | ~100ms |
...
- **Integration Tests**: Test multiple services composing correctly. Uses mock providers but tests real orchestration logic.

- **CLI E2E Tests**: Test the compiled CLI binary end-to-end with `--mock` flag. Validates argument parsing, file I/O, output formats, exit codes. Tests focus on generic workflow execution rather than workflow-specific schemas.
...
### v0.5.1 Design Changes

The v0.5.1 refactor introduced:

1. **Workflow-as-Markdown**: Workflows defined in `workflows/{id}.md` with YAML frontmatter
2. **Generic CLI Format**: `looplia run <workflow-id> --file <path>`
3. **Workflow-Agnostic Testing**: Tests validate generic workflow execution, not specific output schemas
4. **Reduced Complexity**: `parseSimpleYaml` refactored from 67 to <15 complexity
...
| `packages/provider/test/` | 10 files | Unit |
| **Total** | **17 files** | |
...
|------|----------|-------------|
| `e2e/cli.test.ts` | CLI E2E | Full CLI binary execution (global commands, run) |
...
**CLI Commands (v0.5.1):**
- `init` - Initialize/refresh workspace
- `run <workflow-id>` - Execute workflow on content
...
| File | Category | Description |
|------|----------|-------------|
| `domain/validation.test.ts` | Unit | Zod schema validation for ContentSummary, ContentItem, UserProfile, SessionManifest, PipelineDefinition |
...
**Removed in v0.5.1:**
- `integration/full-pipeline.test.ts` - WritingKit-specific integration test
- `services/writing-kit-engine.test.ts` - WritingKit orchestration test
- `WritingIdeas validation` - Removed from validation.test.ts

**v0.5.1 Schema Tests:**
- `SessionManifestSchema` - Session state tracking
- `PipelineDefinitionSchema` - Workflow YAML validation
- `PipelineOutputSchema` - Workflow output validation
...
| `test-markdown` | `ai-healthcare.md` | `run writing-kit` | JSON output + contentId |
| `test-vtt` | `EvtPBaaykdo.en.vtt` | `run writing-kit` | JSON output + contentId |
| `test-srt` | `CBneTpXF1CQ.srt` | `run writing-kit` | JSON output + contentId |
...
# Run workflow (v0.5.1 command format)
docker run --rm \
  --env-file .env \
  ...
  --output /home/looplia/.looplia/output.json
```
...
Verify JSON structure is valid and has contentId:

```bash
# Check valid JSON with contentId
jq -e '.contentId' output.json
```

#### Level 2: Workflow-Specific Validation

For workflows that define validation criteria in their frontmatter:

```bash
# Run validator script
bun run plugins/looplia-writer/skills/workflow-validator/scripts/validate.ts \
  output.json '{"required_fields":["contentId","summary"]}'
```
...
```yaml
- name: Evaluate workflow outputs with Claude Code
  uses: anthropics/claude-code-action@v1
  with:
    claude_code_oauth_token: ${{ secrets.CLAUDE_CODE_OAUTH_TOKEN }}
    prompt: |
      Evaluate workflow outputs from 3 source types.

      Criteria (1 pt each): Accuracy, Completeness, Ideas Quality, Outline Quality

      Sources:
      - Markdown: test-workspace/test-results-markdown/
      - VTT: test-workspace/test-results-vtt/contentItem/*/output.json
      - SRT: test-workspace/test-results-srt/contentItem/*/output.json
...
- On merge to main: Basic tests (JSON output + contentId)
...
### Testing Workflow Configuration (v0.5.1)

```typescript
import { PipelineDefinitionSchema } from "@looplia-core/core";

describe("PipelineDefinition validation", () => {
  it("should accept valid workflow definition", () => {
    const workflow = {
      name: "writing-kit",
      description: "Build a writing kit",
      outputs: {
        summary: { artifact: "summary.json", agent: "content-analyzer" },
        ideas: { artifact: "ideas.json", agent: "idea-generator", requires: ["summary"] },
        "writing-kit": { artifact: "writing-kit.json", agent: "kit-builder", requires: ["ideas"], final: true }
      }
    };

    const result = PipelineDefinitionSchema.safeParse(workflow);
    expect(result.success).toBe(true);
  });
});
```

### Testing Session Manifest (v0.5.1)

```typescript
import { SessionManifestSchema } from "@looplia-core/core";
...
### Workflow-Agnostic CLI E2E Testing (v0.5.1)

Tests focus on generic workflow behavior, not workflow-specific schemas:

```typescript
it("should execute workflow and return valid output", async () => {
  const result = await execCLI([
    "run", "writing-kit", "--file", inputFile, "--mock"
  ]);

  expect(result.exitCode).toBe(0);

  // Generic checks (not workflow-specific)
...
#### Complexity lint errors

```bash
# Solution: Extract helper functions
# parseSimpleYaml was refactored in v0.5.1 as an example
# Use state objects and small, focused functions
```
...
3. **Check streaming events:**
   ```bash
   DEBUG=looplia:* looplia run writing-kit --file article.md
   ```
...
*This test plan is maintained by the Looplia-Core team for v0.5.1.*
