...
This document describes the test architecture for Looplia-Core v0.5.0, aligned with Clean Architecture principles and the Pipeline-as-Configuration design.
...
**CLI Commands (v0.5.0):**
- `init` - Initialize/refresh workspace (was: `bootstrap`)
- `run` - Execute pipeline to build writing kit (was: `kit`)
...
| `domain/validation.test.ts` | Unit | Zod schema validation for ContentSummary, ContentItem, UserProfile, WritingIdeas, SessionManifest, PipelineDefinition |
...
**v0.5.0 Schema Additions:**
- `SessionManifestSchema` - Session state tracking
- `PipelineDefinitionSchema` - Pipeline YAML validation
- `PipelineOutputSchema` - Pipeline output validation
...
| `test-markdown` | `ai-healthcare.md` | `run` | Schema + quality metrics |
| `test-vtt` | `EvtPBaaykdo.en.vtt` | `run` | WritingKit schema |
| `test-srt` | `CBneTpXF1CQ.srt` | `run` | WritingKit schema |
...
# Run writing kit pipeline (v0.5.0 command)
docker run --rm \
  --env-file .env \
...
TLDR_WORDS=$(jq -r '.summary.tldr' kit.json | wc -w)
[ "$TLDR_WORDS" -ge 50 ] && [ "$TLDR_WORDS" -le 200 ]

# Bullet count (3-7)
BULLET_COUNT=$(jq '.summary.bullets | length' kit.json)
...
### Testing Pipeline Configuration (v0.5.0)
...
### Testing Session Manifest (v0.5.0)
...
looplia run --file article.md --mock
```
...
3. **Check streaming events:**
   ```bash
   DEBUG=looplia:* looplia run --file article.md
   ```
...
*This test plan is maintained by the Looplia-Core team for v0.5.0.*
