# workflow-execution Specification Delta

## Purpose

Defines how workflow execution completes and returns results without SDK StructuredOutput enforcement.
...
- **AND** only custom hooks (stop-guard.sh) SHALL control stop behavior
...
- **THEN** stop-guard.sh SHALL find steps to validate
- **AND** SHALL block if any step is not validated

### Requirement: Stop Hook Continuation Prompts

The stop-guard.sh hook SHALL return actionable continuation prompts when blocking.

#### Scenario: Missing output files
- **GIVEN** stop-guard.sh detects missing output files
...
- **GIVEN** stop-guard.sh detects steps with `validated: false`
- **WHEN** blocking the stop request
- **THEN** the `reason` field SHALL contain an actionable continuation prompt
...
- **GIVEN** stop-guard.sh returns a block decision with a reason
