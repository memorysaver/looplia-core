...
/**
 * Build agentic prompt for summarization (v0.5.2 sandbox approach)
 *
 * The agent:
 * 1. Reads CLAUDE.md for full task instructions
 * 2. Invokes content-analyzer subagent via Skill
 * 3. Reads the generated outputs/summary.json output file
 * 4. Returns the JSON as structured output
 *
 * Sandbox structure (v0.5.2):
 * - sandbox/{id}/inputs/content.md: original content to process
 * - sandbox/{id}/outputs/summary.json: generated by content-analyzer subagent
 */
function buildMinimalSummarizePrompt(sandboxId: string): string {
  return `Task: Analyze and summarize content.

Use the content-analyzer subagent to process: sandbox/${sandboxId}/inputs/content.md

The subagent will:
1. Read content from sandbox/${sandboxId}/inputs/content.md
2. Perform deep analysis using skills
3. Write results to sandbox/${sandboxId}/outputs/summary.json

After the subagent completes, read sandbox/${sandboxId}/outputs/summary.json and return its contents as the structured output.`;
}
