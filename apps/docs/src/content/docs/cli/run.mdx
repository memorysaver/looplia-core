...
| `--output <dir>` / `-o` | Copy outputs to specified directory |
...
```bash
# Copy outputs to specific directory
looplia run writing-kit --file article.md --output ./results

# Copy to absolute path
looplia run writing-kit --file article.md -o /tmp/outputs
```

<Aside type="note">
By default, outputs are copied to the current working directory. Use `--output` to specify a different location, or set `LOOPLIA_OUTPUT_DIR` environment variable.
</Aside>
...
4. **Output Generation**
   ...
   - Copies outputs to destination directory (default: current working directory)
