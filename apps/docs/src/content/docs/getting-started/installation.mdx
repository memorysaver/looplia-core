...
```bash
# Install globally
bun install -g @looplia/looplia-cli
...
```bash
# Install globally
npm install -g @looplia/looplia-cli

# Initialize workspace
looplia init
```
  </TabItem>
  <TabItem label="npx/bunx (no install)">
```bash
# Using bunx
bunx @looplia/looplia-cli init

# Or using npx
npx @looplia/looplia-cli init
```
  </TabItem>
...
- **Skills** from default marketplaces ([Anthropic](https://github.com/anthropics/anthropic-agent-skills), [ComposioHQ](https://github.com/ComposioHQ/awesome-claude-skills))
...
```
~/.looplia/
├── registry/               # Skill catalog and sources
│   ├── skill-catalog.json  # Skills from marketplaces
│   └── sources.json        # Configured registry sources
...
- [Skill Registry](/registry/skill-registry/) — Discover 56+ available skills
