...
Steps with `validated: true` are skipped when resuming a workflow. See [Validation System](/reference/validation/) for details on how validation criteria work and how the stop-guard enforces completion.
