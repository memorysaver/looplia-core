...
1. **Config file `authToken`** (highest priority) — set via `looplia config provider set auth-token`
2. **Endpoint-specific env var** — `ZENMUX_API_KEY` for ZenMux, `ANTHROPIC_API_KEY` for Anthropic
3. **Default env var** (`ANTHROPIC_API_KEY`)
