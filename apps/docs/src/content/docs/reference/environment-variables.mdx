...
<Aside>
*Either `ANTHROPIC_API_KEY`, `ZENMUX_API_KEY`, or `OPENROUTER_API_KEY` is required, depending on your provider configuration. Alternatively, use `CLAUDE_CODE_OAUTH_TOKEN` with subscription presets. For Ollama, `OLLAMA_API_KEY` defaults to "ollama" if not set.
</Aside>
...
| `OPENROUTER_API_KEY` | Used when provider is `openrouter` (auto-mapped to `ANTHROPIC_API_KEY` for SDK) |
| `OLLAMA_API_KEY` | Used when provider is `ollama` (auto-mapped to `ANTHROPIC_API_KEY` for SDK, defaults to "ollama") |

### Auto-Mapping

Looplia automatically maps provider-specific API keys to `ANTHROPIC_API_KEY` for SDK compatibility:
...
3. **Endpoint-specific env var** — `ZENMUX_API_KEY` for ZenMux, `OPENROUTER_API_KEY` for OpenRouter, `OLLAMA_API_KEY` for Ollama, `ANTHROPIC_API_KEY` for Anthropic
