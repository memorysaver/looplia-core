...
If validation fails, the step is marked incomplete and can be retried. See [Validation System](/reference/validation/) for all available criteria and how validation enforcement works.
...
3. **Execute Steps** — Run each step through the Skill Executor (separate context per step)
